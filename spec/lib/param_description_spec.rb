...
describe Apipie::ParamDescription do
  ...
  describe 'validator selection' do
    ...
    it 'should throw exception on unknown validator' do
      expect do
        Apipie::ParamDescription.new(method_desc, :param, :unknown)
      end.to raise_error(RuntimeError, /Validator.*not found/)
    end
    ...
    it 'should not allow proc validators on response_only params' do
      expect do
        Apipie::ParamDescription.new(method_desc, :param, lambda { |_val|
                                                            true
                                                          }, only_in: :response)
      end.to raise_error(RuntimeError, /Proc validators not supported.*/)
    end
  end
  ...
  describe 'required params on given actions' do
    ...
    context 'when the param is required for multiple actions' do
      it 'should set param as required if it match current action' do
        param = Apipie::ParamDescription.new(method_desc, :required, String, 'description',
                                             'required' => %i[update create])
        expect(param.required).to be true
      end
    end
