module Apipie
  ...
  class ParamDescription
    attr_reader :method_description, :name, :desc, :allow_nil, :allow_blank, :validator, :options, :metadata, :show,
                :as, :validations, :response_only, :request_only, :additional_properties, :is_array
    ...
    def initialize(method_description, name, validator, desc_or_options = nil, options = {}, always_valid = false, &block)
      ...
      @options = options
      @from_concern = @options[:param_group][:from_concern] if @options[:param_group]
      ...
      if (!@response_only && !@request_only) && @options[:only_in].present?
        raise ArgumentError, "'#{@options[:only_in]}' is not a valid value for :only_in"
      end
      ...
      if validator
        if (validator != Hash) && (validator.is_a? Hash) && (validator[:array_of])
          ...
          options.merge!(rest_of_options.select { |k, _v| k != :array_of })
        ...
        puts @validator
        if (@validator.is_a? Validator::ProcValidator) && @response_only
          raise "Proc validators not supported on param #{method_description.id}:#{name}"
        end
