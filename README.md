...
:   How the resource will be referenced in Apipie (paths, `see` command
    etc.); by default [controller_name.downcase]{.title-ref} is used.
...
:   Short description of the resource (it\'s shown on both the list of
...
:   Common params for all methods defined in controller/child
    controllers.

returns

:   Common responses for all methods defined in controller/child
    controllers.
...
:   What versions does the controller define the resource. (See
...
:   In case of versioning, this sets app info description on a
    per_version basis.
...
:   Used by the processing functionality to change the name of a key
    params.
...
:   Parameter is hidden from documentation when set to false (true by
    default)
...
[create]{.title-ref} or [update]{.title-ref} routine, it would be quite
frustrating to have to define a completely separate `:param_group` to
describe the output of the [show]{.title-ref} routine.
...
[describe_own_properties]{.title-ref} class method which returns a
description of the properties such a class would expose. It is then
possible to specify that class in the [returns]{.title-ref} statement
instead of a [param_group]{.title-ref}.

The [describe_own_properties]{.title-ref} method is expected to return
an array of [Apipie::prop]{.title-ref} objects, each one describing a
single property.
...
The [Apipie::prop]{.title-ref} function expects the following inputs:
...
using [update_api]{.title-ref} method. The params defined in this block
...
:   Either [:raise]{.title-ref} or [:skip]{.title-ref}. If
    [validate_key]{.title-ref} fails, raise error or delete the
    non-validated key from the params and log the key
    ([:raise]{.title-ref} by default)
...
:   Determines the strategy to identity the correct controller action.
    Needs to be a class that implements a
    [.call(controller)]{.title-ref} method
...
:   An object providing the translation from the Rails routes to the
    format usable in the documentation when using the [api!]{.title-ref}
...
:   [allow_blank: false]{.title-ref} was incorrectly ignored up until
...
routes defined in the Rails application, by using the [api!]{.title-ref}
keyword in the DSL.
...
validation or return some text about what is wrong otherwise. \_Don\'t
...
:   Specify the type of items. If not given it accepts an array of any
    item type
...
Assert [things]{.title-ref} is an array of any items

``` ruby
param :things, Array
```

Assert [hits]{.title-ref} must be an array of integer values

``` ruby
param :hits, Array, of: Integer
```

Assert [colors]{.title-ref} must be an array of valid string values
...
:   Instance of Apipie::ParamDescription contains all given information
    about the validated parameter.
...
:   Block converted into Proc, use it as you desire. In this example
    nil.

If your validator includes valid values that respond true to
[.blank?]{.title-ref}, you should also define:
...
specified with the [api_version]{.title-ref} DSL keyword. When not
specified, the resource belongs to [config.default_version]{.title-ref}
(\"1.0\" by default)
...
[index]{.title-ref} belongs only to version \"1\". Also, inheritance
...
default version ([Apipie.configuration.default_version]{.title-ref}) is
...
[versions]{.title-ref} keyword. It specifies the versions the example is
...
# Static Swagger (OpenAPI 2.0) files {#Swagger}
...
:   missing short description for method

101

:   added missing / at beginning of path

102

:   no return codes specified for method

103

:   a parameter is a generic Hash without an internal type specification

104

:   a parameter is an \'in-path\' parameter, but specified as \'not
    required\' in the DSL

105

:   a parameter is optional but does not have a default value specified

106

:   a parameter was ommitted from the swagger output because it is a
    Hash without fields in a formData specification

107

:   a path parameter is not described

108

:   inferring that a parameter type is boolean because described as an
    enum with \[false,true\] values
